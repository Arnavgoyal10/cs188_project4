...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # Add edges: Pacman -> Observation0, Ghost0 -> Observation0, Pacman -> Observation1, Ghost1 -> Observation1
    edges = [(PAC, OBS0), (GHOST0, OBS0), (PAC, OBS1), (GHOST1, OBS1)]

    # Create all possible position tuples for agents
    all_positions = []
    for x in range(X_RANGE):
        for y in range(Y_RANGE):
            all_positions.append((x, y))

    # Create all possible observation values (non-negative distances with noise)
    # Observations are noisy Manhattan distances, so they can be 0 to max_distance + MAX_NOISE
    max_distance = X_RANGE + Y_RANGE - 2  # Maximum possible Manhattan distance
    all_observations = list(
        range(max_distance + MAX_NOISE + 1)
    )  # 0 to max_distance + MAX_NOISE
    ...
    variableDomainsDict[OBS0] = all_observations
    variableDomainsDict[OBS1] = all_observations
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(
        ...
        # Iterate through elimination order
        for eliminationVariable in eliminationOrder:
            # Join all factors that contain this variable
            currentFactorsList, joinedFactor = joinFactorsByVariable(
                currentFactorsList, eliminationVariable
            )

            # Check if joinedFactor is None (no factors to join)
            if joinedFactor is None:
                continue

            # If the joined factor has more than one unconditioned variable, eliminate the variable
            ...
            # Otherwise, discard the factor (it would eliminate to 1)

        # Join any remaining factors
        if len(currentFactorsList) > 1:
            finalFactor = joinFactors(currentFactorsList)
        elif len(currentFactorsList) == 1:
            finalFactor = currentFactorsList[0]
        else:
            # No factors left, create an empty factor
            finalFactor = None

        # Normalize to get conditional probability
        if finalFactor is not None:
            result = normalize(finalFactor)
        else:
            # Handle edge case where no factors remain
            result = None
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        # Find the key corresponding to this random value
        cumulative = 0.0
        ...
        # Fallback (shouldn't reach here if distribution is valid)
        return list(self.keys())[0] if self else None
        "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(
        ...
        """
        "*** YOUR CODE HERE ***"
        # Special case: if ghost is in jail, observation must be None
        ...
        # Special case: if observation is None, ghost must be in jail
        if noisyDistance is None:
            if ghostPosition == jailPosition:
                return 1.0
            else:
        ...
        # Use the provided function to get observation probability
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # Update beliefs using Bayes' rule: P(ghost_position | observation) ∝ P(observation | ghost_position) * P(ghost_position)
        for position in self.allPositions:
            # Get the observation probability
            obsProb = self.getObservationProb(
                observation, pacmanPosition, position, jailPosition
            )
            # Update belief: multiply by observation probability
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Create a new belief distribution for the next time step
        ...
        for oldPos in self.allPositions:
            ...
            # For each possible new position, add the contribution from this old position
